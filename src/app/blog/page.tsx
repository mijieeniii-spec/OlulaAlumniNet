...
function BlogCard({ post, onClick }: { post: BlogPost; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="bg-white border border-[#E5E7EB] rounded-2xl overflow-hidden cursor-pointer hover:border-[#32B4C5]/40 hover:shadow-md transition-all group"
    >
      {post.imageUrl && (
      ...
      <div className="p-5">
        ...
        <h3 className="text-[#0E172B] font-semibold mb-2 group-hover:text-[#32B4C5] transition-colors line-clamp-2">
          {post.title}
        </h3>
        ...
        <div className="flex items-center gap-3 pt-3 border-t border-[#E5E7EB]">
          <img
            ...
            alt={post.author}
            className="w-7 h-7 rounded-full bg-[#E5E7EB]"
          />
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white border border-[#E5E7EB] rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-7">
          <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 bg-white border border-[#E5E7EB] rounded-full p-1.5 shadow-sm">
            <X className="w-4 h-4" />
          </button>
          ...
          {post.tags.length > 0 && (
            <div className="mt-6 pt-5 border-t border-[#E5E7EB] flex gap-2 flex-wrap">
              {post.tags.map((t) => (
                <span key={t} className="bg-[#F3F5F6] border border-[#E5E7EB] text-[#647588] text-xs px-3 py-1 rounded-full">
                  #{t}
                </span>
              ))}
            </div>
        ...
        <div className="flex gap-2 flex-wrap mb-8 items-center">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
              !activeCategory ? "bg-[#32B4C5] text-white" : "bg-white border border-[#E5E7EB] text-[#647588] hover:border-[#32B4C5]/40"
            }`}
          >
            Бүгд ({posts.length})
          </button>
